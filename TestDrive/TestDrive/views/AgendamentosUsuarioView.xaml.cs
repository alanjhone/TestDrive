    ...
    public partial class AgendamentosUsuarioView : ContentPage
    {
        ...
        protected override void OnAppearing()
        {
            ...
            MessagingCenter.Subscribe<Agendamento>(this, "AgendamentoSelecionado", async (agendamento) =>
            {
                ...
                {
                    var reenviar = await DisplayAlert("Reenviar Agendamento", "Desenja reenviar o agendamento?", "SIM", "NÃO");

                    if (reenviar)
                    {
                        AgendamentoService agendamentoService = new AgendamentoService();
                        await agendamentoService.SalvarAgendamentoServidor(agendamento);
                        this._viewModel.AtualizarListagem();
                    }
                }
            });
            ...
            {
                await DisplayAlert("Reenviar", "Operação realizada com sucesso!", "OK");
            });

            MessagingCenter.Subscribe<Agendamento>(this, "FalhaAgendamento", async (agendamento) =>
            {
                await DisplayAlert("Reenviar", "Não foi possível realizar esta operação!", "OK");
            });
